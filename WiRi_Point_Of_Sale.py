...
pos=Tk()
pos.geometry('750x500+320+150')
pos.configure(background='cadetblue')
pos.overrideredirect(1)
imglogo = ImageTk.PhotoImage(Image.open('assets/createlogosmall.png'))
logo_label = Label(pos, image=imglogo,bg='cadetblue')
logo_label.place(x=80, y=100)
...
l1=Label(pos,text="WiRi",font=("Garamond",50,"bold"),bg="cadetblue")
l1.place(x=230,y=175)

l2=Label(pos,text="RETAIL SYSTEMS",font=("Garamond",25,"bold"),bg="cadetblue")
l2.place(x=380,y=220)

l3=Label(pos,text="v1.0.1",font=("Garamond",12),bg="cadetblue")
l3.place(x=675,y=235)

l4=Label(pos,text="Loading...",font=("Garamond",12,"bold"),bg="cadetblue")
#l4.place(x=400,y=320)

progress=Progressbar(pos,style="red.Horizontal.TProgressbar",orient=HORIZONTAL,length=750,mode="determinate")

def bar(*args):
    l4=Label(pos,text="Loading...please wait",fg="white",bg="cadetblue")
...
progress.place(x=0,y=495)

bt=Button(pos,width=10,text="Proceed>>",command=bar,border=0,fg="black",bg="cadetblue")
bt.place(x=350,y=330)
    ...
    def login(*args):
        if usernameEn.get() == "" or passwordEn.get() == "":
            ...
            try:
                ...
                con = mysql.connect(host=host_address, user='root', password='', database='retail-system2       ')
                cur = con.cursor()
                cur.execute('select * from employee where name=%s and password=%s',
                            (usernameEn.get(), passwordEn.get()))
                row = cur.fetchone()
                if row == None:
                    ...
                    discountlabel = Label(discountframe, text="Discount Amount", font=("open sans", 12, "bold"),
                                          fg='blue')
                    discountlabel.place(x=0, y=0)

                    discountmoney = Label(discountframe, text=main_currency + " 0.00", font=("open sans", 18, "bold"),
                                          fg='blue')
                    ...
                    totalmoney = Label(totalframe, text=main_currency + " 0.00", font=("open sans", 18, "bold"),
                                       fg='blue')
                    ...
                    def payments(*args):
                        ...
                        try:
                            ...
                            def addupdate():
                                ...
                                try:
                                    payamountfig.config(text=main_currency + " {:,.2f}".format(float(tot)))
                                except:
                                    payamountfig.config(text=main_currency + " 0.00")
                            ...
                            def select():
                                ...
                                if payen.get() == '' or amounten.get() == '' or trv.focus() == '':
                                    ...
                                    for i in range(len(best)):
                                        ...
                                        ided = str(best[i][0])
                                        cursor.execute(
                                            "select diff_cost from product where product_id='" + str(proid2) + "'")
                                        ...
                                        cumulative = '100'
                                        cursor.execute(
                                            "insert into inventory_audit values('" + proid22 + "', '" + sale + "', '" + proquan + "', '" + total_cost1 + "', '" + cumulative + "', '" + date + "' )  ")
                                        cursor.execute("commit");
                                    ...
                                    cursor.execute("insert into profitter(pro_id, total_cost, date) values('" + str(
                                        proid2) + "', '" + str(total_cost) + "','" + date + "')")
                                    cursor.execute("commit")
                                    con.close()
                                    con.close()
                                    arrow_down = '↓'
                            ...
                            def decrease():
                                ...
                                for i in range(len(main_sales)):
                                    ...
                                    cursor.execute(
                                        "select product_id from sales where invoice_id ='" + last_main + "' ")
                                    ...
                                    for i in range(len(main_id)):
                                        ...
                                        cursor.execute(
                                            "update product set unit_in_stock= unit_in_stock - '" + str(
                                                main_sales[i][0]) + "' where product_id='" + str(main_id[i][0]) + "'")
                                        cursor.execute("commit");

                                        cursor.execute(
                                            "update product set total_cost= unit_in_stock * cost_price where product_id='" + str(
                                                main_id[i][0]) + "'")
                                        cursor.execute("commit");
                            ...
                            payamount = Label(payinfo, text="Amount Payable", font=("cansalas", 10), fg="white",
                                              bg="black")
                            payamount.place(x=5, y=5)

                            payamountfig = Label(payinfo, text=main_currency + " 0.00", font=("cansalas", 10),
                                                 fg="white", bg="black")
                            payamountfig.place(x=30, y=25)

                            amounttend = Label(payinfo, text="Amount Tendered", font=("cansalas", 10), fg="white",
                                               bg="black")
                            amounttend.place(x=200, y=5)

                            amounttendfig = Label(payinfo, text=main_currency + " 0.00", font=("cansalas", 10),
                                                  fg="white", bg="black")
                            amounttendfig.place(x=220, y=25)

                            amountchange = Label(payinfo, text="Amount Change", font=("cansalas", 10), fg="white",
                                                 bg="black")
                            amountchange.place(x=400, y=5)

                            amountchangefig = Label(payinfo, text=main_currency + " 0.00", font=("cansalas", 10),
                                                    fg="white", bg="black")
                    ...
                    def transaction():
                        ...
                        try:
                            ...
                            transmodestatus = Label(trasnmode, text="Page Not Available !",
                        ...
                        try:
                            ...
                            def search():
                                try:
                                    ...
                                    cursor = mydb.cursor()
                                    cursor.execute(
                                        "select * from product where product_code like '%" + secentry.get() + "%' and name like '%" + thirdentry.get() + "%' ")
                                    rows = cursor.fetchall()
                                    update(rows)
                    ...
                    def voidsale(*args):
                        ...
                        try:
                            ...
                            warn_label = Label(void_sale, text="ARE YOU SURE YOU WOULD LIKE TO",
                                               font=("georgia", 15, "bold"), bg="cadetblue")
                            warn_label.place(x=5, y=70)
                            ...
                            yes_butt = Button(void_sale, text="Yes", width=7, height=3, font=("georgia", 14),
                                              command=yes)
                            yes_butt.place(x=50, y=280)
                    ...
                    def transearch(*args):
                        ...
                        try:
                            ...
                            translabel = Label(transframe, text="Transaction Search", font=("open sans", 20),
                                               bg='lightblue')
                            ...
                            transfirst = Frame(transcriteria, width=450, height=50, relief=RIDGE, bd=1,
                                               bg='floral white')
                            ...
                            def search():
                                ...
                                try:
                                    ...
                                    cursor.execute(
                                        "SELECT * FROM `transaction` WHERE `transaction_id` LIKE '" + transsecentry.get() + "'  AND `date_recorded` BETWEEN '" + date_cal + "' AND '" + date_cal2 + "'")
                            ...
                            def view_sales():
                                ...
                                try:
                                                    ...
                                                    for i in range(len(main_id)):
                                                        ...
                                                        cursor.execute(
                                                            "update product set unit_in_stock= unit_in_stock + '" + str(
                                                                ...
                                                                main_id[i][0]) + "'")
                                                        cursor.execute(
                                                            "DELETE FROM `transaction` WHERE `transaction`.`transaction_id` = '" + trans_id + "' ")
                                    ...
                                    salesfirst = Frame(salescriteria, width=450, height=50, relief=RIDGE, bd=1,
                                                       bg='floral white')
                                    salesfirst.place(x=0, y=0)

                                    salesfirstlabel = Label(salesfirst, text="Search Criteria", font=("open sans", 14),
                                                            bg="floral white")
                                    ...
                                    salesthird2 = Frame(salescriteria, width=225, height=50, relief=RIDGE, bd=1,
                                                        bg="white")
                                    ...
                                    salesfour2 = Frame(salescriteria, width=225, height=50, relief=RIDGE, bd=1,
                                                       bg="white")
                                    ...
                                    salesfive2 = Frame(salescriteria, width=225, height=50, relief=RIDGE, bd=1,
                                                       bg="white")
                                    ...
                                    salessevenlabel2 = Label(salesseven2, text=trans_total, font=("open sans", 13),
                                                             fg="red")
                                    ...
                                    salestrv1 = ttk.Treeview(saleswrapper, columns=(1, 2, 3, 4, 5, 6, 7),
                                                             show="headings",
                                                             height=26)
                                    ...
                                    for i in range(len(salesrows)):
                                        ...
                                        cursor.execute(
                                            "select name from product where product_id = '" + spro_ids + "' ")
                                        ids = cursor.fetchall()
                                    ...
                                    salesposbut2 = Button(salessearchbutt, width=13, height=7,
                                                          text=("Sale\n""Exchange"),
                                                          font=('open sans', 10), command=None)
                                    ...
                                    salesposbut4 = Button(salessearchbutt, width=13, height=7,
                                                          text=("Special Order\n""Edit"),
                                                          font=('open sans', 10), state=DISABLED)
                                    salesposbut4.place(x=360, y=2)

                                    salesposbut5 = Button(salessearchbutt, width=13, height=7,
                                                          text=("Special Order\n""Payment"),
                                                          font=('open sans', 10), state=DISABLED)
                                    ...
                                    salesposbut8 = Button(salessearchbutt, width=13, height=7,
                                                          text=("Print Transaction"),
                                                          font=('open sans', 10))
                                    salesposbut8.place(x=840, y=2)

                                    asalesposbut9 = Button(salessearchbutt, width=13, height=7,
                                                           text=("F9\n""View\n""Inventory"),
                                                           font=('open sans', 10))
                    ...
                    posbut1 = Button(butframe, width=13, height=7, text=("F1\n""Transaction\n""Mode"),
                                     font=('open sans', 10), command=transaction)
                    ...
                    posbut5 = Button(butframe, width=13, height=7, text=("F5\n""Transaction\n""Search"),
                                     font=('open sans', 10), command=transearch)
